...
const TERMS: { term: string; weight: string; meaning: string }[] = [
  ...
  },
  {
    ...
    weight: "× 0.20",
    meaning:
      "Starts at 0.6. Set to 0 (excluded) when the step needs judgment and the technique would act; 0.4 for Assist on a judgment step; 0.9 for automation on a step with no judgment content. Raised by 0.3 for a contextual pointer when the step's fields are hidden; halved for a worked example when no field implies a format; capped at 0.5 for acting on a commit step, since approval is required anyway. Raised by 0.35 (first choice) or 0.15 when the technique is the minimal intervention for the observed interaction state (section 03), and multiplied by 0.3 for a pointer under decision uncertainty, because the control was already found. The person's preference adjusts it too: +0.15 for guidance under teach me, +0.2 for assist and act under just do it, ×0.6 for guidance under stay out of the way. Get It Done, or performance mode, halves instruction (except clarify consequence and policy clarification) and adds 0.25 to assist and act; recovery mode after a failed run adds 0.2 to worked example, prefill and inline explanation.",
  },
  {
    term: "evidenceWeight",
    weight: "× 0.15",

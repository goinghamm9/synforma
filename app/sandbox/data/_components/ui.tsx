...
export function RlsBadge({ enabled, label }: { enabled: boolean; label?: string }) {
  return <Badge tone={enabled ? "green" : "amber"}>{enabled ? (label ?? "RLS enabled") : "Unrestricted"}</Badge>;
}

export function Code({ children, className }: { children: React.ReactNode; className?: string }) {
  return <code className={cn(s.code, className)}>{children}</code>;
}
...
export const inputMonoClass = s.controlMono;
export const selectClass = s.select;
export const textareaClass = s.textarea;
export const textareaMonoClass = s.textareaMono;
